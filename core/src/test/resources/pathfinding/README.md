...
## `golden/bush_goblins.json`, `golden/brawler_goblins.json`, `golden/gift_knight.json`, `golden/abort_instigator.json` and `golden/witch_hooks.json` - characters an action spawns
...
`BattleActionSpawnRunTest` plays each file through the battle and holds it to every action run, spawn, drop and group link and unlink, every unit record, every tower's lock, every event and every projectile position.
