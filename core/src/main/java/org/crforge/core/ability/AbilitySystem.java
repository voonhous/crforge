...
public class AbilitySystem {
    ...
    for (Entity entity : aliveEntities) {
      if (entity instanceof Troop troop) {
        ...
        switch (ability.getData().type()) {
          ...
          case BUFF_ALLY -> updateBuffAlly(troop, ability, deltaTime);
          case RANGED_ATTACK -> updateRangedAttack(troop, ability, deltaTime);
        }
  ...
  private void updateTunnel(Troop troop, AbilityComponent ability, float deltaTime) {
  ...
  // -- BUFF_ALLY (GiantBuffer) --

  /**
   * Updates the BUFF_ALLY cycle timer for a GiantBuffer troop. After the initial action delay, the
   * GiantBuffer selects up to N closest friendly troops within search range and applies a damage
   * buff to them every cooldown seconds.
   */
  private void updateBuffAlly(Troop troop, AbilityComponent ability, float deltaTime) {
   ...
   * after persist duration.
   */
  private void tickGiantBuffs(float deltaTime) {
    for (Entity entity : gameState.getAliveEntities()) {
      if (!(entity instanceof Troop troop)) {
        continue;
      }
   ...
   */
  private void updateRangedAttack(Troop troop, AbilityComponent ability, float deltaTime) {
