...
class TransformationSystem {
  ...
  private void transform(Troop old, TransformationConfig config) {
    ...
    int scaledMaxHp = LevelScaling.scaleCard(kamikazeStats.getHealth(), level);
    int scaledDamage = LevelScaling.scaleCard(kamikazeStats.getDamage(), level);
    ...
    if (kamikazeStats.getDeathSpawnProjectile() != null) {
      ...
      int scaledProjDamage = LevelScaling.scaleCard(deathProjStats.getDamage(), level);
      ...
      spawner = SpawnerComponent.builder().deathSpawnProjectile(scaledProj).level(level).build();
    }
