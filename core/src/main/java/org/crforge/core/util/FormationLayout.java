...
/**
 * Calculates circular formation offsets for multi-unit deployments.
 * Ported from the Python parser's _calculate_offsets algorithm.
 *
...
public final class FormationLayout {
  ...
  /**
   * Convenience method for troop deploy offsets. Divides the raw summonRadius
   * by TILE_SCALE to convert from CSV units to tile units before calculating.
   *
   * @param index           zero-based index of the unit being placed
   * @param total           total number of units in the formation
   ...
   * @return offset vector (relative to deploy center), rounded to 3 decimal places
   */
  public static Vector2 calculateDeployOffset(int index, int total, float summonRadius,
      float collisionRadius) {
