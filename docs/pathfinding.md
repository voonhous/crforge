...
- **Targeting switches.** The switches the targeting visit reads carry their published values. The reference trajectories were produced with six of them left at zero, which was not a decision about them: a lone unit walking to a tower reaches none of the six, so the trajectories are reproduced identically either way and constrain none of them. What they do for units that reach them - bursts, special loads, wind-up-before-first-hit units, pending damage - is therefore held only by the targeting visit's own tests.
...
These are behaviours of the current model that were found by running several units at once. Each is recorded on every smoke run and asserted in a disabled test named after it; nothing was changed to hide them and no live invariant was weakened. The first three are settled as the standard game's behaviour: the battle engine, run on the same scenarios, shows them with the same counts and first ticks as the reference composition, no supplied answer is left on any of their paths, and the two swarm runs are held tick for tick as references (`skeleton_army_bridge`, `skeleton_army_corner`). The fourth is this engine's own deployment defect.

1. **A walking troop with a target has no route for one tick.** Two Knights facing each other, at ticks 67 and 69 for both of them. The follower pops the last route node when a displacement reports arrival, and route preparation only rebuilds the route at the head of the next visit, so the route cannot be non-empty at the end of that tick. Reproduce with a BLUE Knight at (3500, 12000), a RED Knight at (3500, 20000), 171 ticks. What does hold, and is asserted live, is that the route is never empty for two ticks running. The swarm scenario does not reach this: no skeleton of that run arrives on the same tick its route runs out. **Settled as behaviour:** the movement visit prepares the route before the follower runs, so a route the follower empties stays empty until the next visit; in the battle engine the two Knights show it on ticks 66 and 68, both of them, as the reference composition does.
2. **A pushed troop walks on water.** Only in the swarm scenario: the swarm pushes its own members sideways off the bridge, and nothing stops them at the water's edge or nudges them back. The displacement has a branch that tests a pushed ground unit's cell for water and nudges it away from the river line, but it is opened only by a mode with a capturable building on the arena, which none of the standard modes is. **Settled as behaviour:** in the standard game a unit pushed off a bridge walks on the water beside it, and even with the branch open the first water entries of the reference runs are the same; the battle engine's bridge swarm puts 29 unit-ticks on water, first on tick 60. Reproduce by playing `skeletonarmy` from BLUE's hand at (3500, 10000) and running 267 ticks. The test pins that it happens, not how often; the other scenarios assert that no troop ever stands on water and pass.
3. **A pushed troop ends up inside a tower footprint.** The swarm scenario shows it. Fifteen skeletons crowd the tower, the push pass sums what surrounds one of them and the displacement applies the average with no test against a building footprint. The routing overlay keeps a *route* out of a building's cells; a push is not routed. The tower pushes back, but its mass is 0, so its share is a single unit averaged with the crowd's. The corner scenario no longer reaches this: its skeletons are steered around the tower they attack. **Settled as behaviour:** the battle engine's bridge swarm has 44 unit-ticks inside the enemy princess tower, all attacking, first on tick 153, as its reference records; the aligned static neighbours check the push pass asks there answers no, since only one static entity stands near.
4. **A formation deployed at an arena corner leaves part of itself outside the arena.** A Skeleton Army played from BLUE's hand at (500, 1500): six of its fifteen places land outside the arena, at (-63, 2766), (-1561, 3355), (-378, 1785), (-1797, 1259), (-587, 5) and (2130, -743). A formation's units are placed at their offsets from the deploy point with no test against the arena; keeping a deploy point inside the arena belongs to deployment, which this mode does not own. Four of the six stand off the routing grid outright, get no route, and from tick 42 break the run's hard invariant against walking toward a target with no route two ticks running. The other two stand within one cell of the left edge, which the cell conversion still reports as the leftmost column, so they hold a route. The one at (-378, 1785) is carried onto the arena while it deploys: a deploying unit is visited by the movement pass, and the grid move snaps it to its cell's lower edge, which for a unit off the grid lies ahead of it. **That snap is the grid move's own rule and is kept as it is**: it agrees with the reference for positions off the grid as well as on it. What is wrong is upstream, that a unit was ever placed where the rule could carry it that far. The other, at (-63, 2766), stays outside. The test pins the five units seen outside, 1425 troop ticks outside the arena over the run, and the four unroutable units exactly.
...
- **The dash and jump branches, and negative speeds**, are implemented but unexercised. Nothing on the current paths produces a negative speed budget. Three pieces of the dash are known to be incomplete rather than merely untested, and are documented on `TargetingVisit` itself: the dash pushback is applied to every entity in reach where the standard game skips one without a movement component, the dash damage index is read from the first slot rather than from the dashing entity's own, and the offset that stops a dash at the two entities' touching edges rather than at the target's centre is not applied.
- **The jump target is computed and discarded.** The follower works out where a jump should land and only announces that it did; nothing downstream can learn the point. No jump-enabled unit is grid-managed today, so nothing reads it, and a unit that jumps stays on the waypoint rules.
- **Six state-visit and state-setter timers have no writer.** The entity state setter carries its interrupt guard and the actions a plain ground unit reaches (the route emptied when the unit stops, prepared the moment it resumes, the target-lost timer cleared when it stops attacking, the deploy countdown cleared when it leaves deployment): the entry and exit actions that would seed the deploy countdown, the morph countdown, the two ability countdowns and the dash flags are not ported, and neither are the state visit's growth, spawner, ability and clone blocks. The blocks that read those timers are ported and correct but inert, and because the omitted blocks also carry early returns, a ported block's reachability is not exactly the standard game's either. The removal-request flag has the same shape: the only ported writer is the lifetime block, so a kamikaze unit that cannot run its removal block, or a spawner past its limit, never asks to be removed. The dash-landing block likewise announces the landing without doing the targeting work the standard game does, and the ability block does not remove the buff the cast leaves behind. None of this is reachable from a plain ground troop, which is all the grid manages.
- **The movement gates are answered from live state, not from a per-tick snapshot.** `GridPathfindingSystem.speedInputs()` rebuilds the speed and gate inputs from the entity's current state, flags and charge progress every time one of them is asked for, while the standard game freezes them once before the visit. Nothing inside the substep loop writes the state, the flags or the charge progress, so the two agree today; a pass that starts writing them during a visit would make them differ.
