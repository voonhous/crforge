...
- **Tower targeting columns** are taken from the simulator rather than hard-coded, so a princess tower carries a sight range of 9500 here where the reference table said 7500. A candidate's own sight range is never read by the attacker's selection, so this is not observable.
...
- `core/src/test/java/org/crforge/core/pathfinding/GridSmokeScenariosTest.java` runs several units at once, 600 ticks or a hundred ticks past the first attack lock, and checks the per-tick invariants on every grid-driven troop; the three disabled tests at the bottom hold the anomalies above.
