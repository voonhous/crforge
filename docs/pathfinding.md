...
- **The per-neighbour contact flag** the avoidance handler reads, and the per-entity enable bit the push pass reads, are both supplied and neither has a documented writer. Both are answered the same way: on for a troop, off for a building or a tower. Troop-to-troop pushing and steering therefore stay live, while a lone unit walking past one of its own towers reproduces the reference trajectory, which the `knight_right_rear` and `knight_behind_king` cases pin. Answering either of them on for a building takes a unit deployed behind its own king tower down the other lane, so this is a load-bearing decision and not a default.
...
1. **A walking troop with a target has no route for one tick.** Two Knights facing each other, at ticks 67 and 69 for both of them. The follower pops the last route node when a displacement reports arrival, and route preparation only rebuilds the route at the head of the next visit, so the route cannot be non-empty at the end of that tick. Reproduce with a BLUE Knight at (3500, 12000), a RED Knight at (3500, 20000), 171 ticks. What does hold, and is asserted live, is that the route is never empty for two ticks running. The swarm scenario no longer reaches this at all: with towers out of the push pass, no skeleton of that run arrives on the same tick its route runs out.
2. **A pushed troop walks on water.** Only in the swarm scenario: the swarm pushes its own members sideways off the bridge, and nothing stops them at the water's edge or nudges them back. The displacement has a branch that does exactly that for a pushed ground unit - a cell test that marks it stuck, a water clamp the stuck mark arms, and a nudge away from the river line - but the whole branch hangs on one battle-level answer that is supplied here as off. Whether the standard game has it on is not settled, so this may be an artefact of that answer and not behaviour. Reproduce by playing `skeletonarmy` from BLUE's hand at (3500, 10000) and running 267 ticks. The test pins that it happens, not how often; the other scenarios assert that no troop ever stands on water and pass.
3. **A pushed troop ends up inside a tower footprint.** The swarm scenario and the corner scenario both show it; the corner scenario pins 59 troop ticks. Fifteen skeletons crowd the tower, the push pass sums what surrounds one of them and the displacement applies the average with no test against a building footprint. The routing overlay keeps a *route* out of a building's cells; a push is not routed. The tower itself does not push back.
4. **A formation deployed at an arena corner leaves part of itself outside the arena.** A Skeleton Army played from BLUE's hand at (500, 1500): six of its fifteen places land outside the arena, at (-63, 2766), (-1561, 3355), (-378, 1785), (-1797, 1259), (-587, 5) and (2130, -743) - 1489 troop ticks outside the arena over the run. A formation's units are placed at their offsets from the deploy point with no test against the arena; keeping a deploy point inside the arena belongs to deployment, which this mode does not own. Four of the six stand off the routing grid outright, get no route, and from tick 42 break the run's hard invariant against walking toward a target with no route two ticks running. The other two stand within one cell of the left edge, which the cell conversion still reports as the leftmost column, so they hold a route; the first step one of them takes to the left snaps it to its cell's lower edge, which for a unit off the grid lies ahead of it, so it is carried 391 units in one tick, onto the arena's edge. **That snap is the grid move's own rule and is kept as it is**: it agrees with the reference for positions off the grid as well as on it. What is wrong is upstream, that a unit was ever placed where the rule could carry it that far. The test pins the six places, the 1489 ticks, the four unroutable units and the one carried unit exactly.
