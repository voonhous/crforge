...
- `BattleKillRunTest` drives the kill run, a Knight destroying the princess tower and then the king tower, and holds the battle to as much of it as the milestone has reached: today the tick of every hit on the princess tower and what the tower stands at after each of them, down to its death on the sixteenth, the hit points every tower and the Knight start the run with, and the damage of one Knight hit, all at the reference's level. An attack starting from zero is credited the whole of a run-down load, so the first hit lands nine ticks after the lock and the rest follow at the hit speed.
...
- Every entity carries hit points and damage at its level, and the towers scale as Common. A hit of a unit without a projectile lands on its target and lowers it; nothing else does, because projectiles, area damage and the buffs that change an amount are not modelled. An entity whose hit points reach zero is dead and the holder drops it at the next cleanup; what a death does beyond that - the rewards, the death spawns, what a destroyed tower does to the match - is not modelled.
