...
- `BattleKillRunTest` drives the kill run, a Knight destroying the princess tower and then the king tower, and holds the battle to the whole of it: every one of the 1253 records, with the Knight's position, state, target, route length and movement budget and the target's remaining hit points; the tick and remaining hit points of every one of the forty hits and both deaths; the removal of a dead tower in the tick it dies and the resume on the tick after; the hit points every tower and the Knight start the run with; and the damage of one Knight hit, all at the reference's level.
- `TrajectoryRecorderTest` plays the same run with a `TrajectoryRecorder` attached to the battle's world and holds the file it writes to the committed reference byte for byte. The recorder writes a character's run in the layout of the reference trajectories, the header, the towers, the events and one compact record per tick, so a run the engine plays can be compared with a reference directly, or become a fixture.
...
- Every entity carries hit points and damage at its level, and the towers scale as Common. A hit of a unit without a projectile lands on its target and lowers it; nothing else does, because projectiles, area damage and the buffs that change an amount are not modelled. An entity whose hit points reach zero is dead and leaves the holder in the closing cleanup of the tick it dies, as the reference run has it; what a death does beyond that - the rewards, the death spawns, what a destroyed tower does to the match - is not modelled.
...
- A removable entity - one that has asked to be removed or has no hit points left - leaves the holder in the closing cleanup of the tick it becomes removable, and every character's selection at the next pre-pass, which clears a reference still held to it; a character that was attacking it therefore takes its next target and resumes on the tick after the death, as the reference run has it. Whether the selections drop it then rather than the validator finding it dead first, whether the side lists drop it in the same cleanup, and whether anything else notices the removal, is not settled.
