...
| M4 (in progress) | Done: one command pass before the entity tick; the placement of a troop card worked out: the map check, the deploy mask, the search, the column, the formation and each unit's start. Next: the command that creates the units, and the stagger they wait through. Deployment as a command: placement, the formation of a multi-unit card, the stagger, the states before the first move, placement validation | multi-unit deployments, including at the arena's edge |
